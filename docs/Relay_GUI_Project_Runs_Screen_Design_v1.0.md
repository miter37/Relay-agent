# Relay GUI — Project Runs 화면 설계 v1.0
...
노드가 많거나 값을 비교할 때를 위한 조밀한 표.

`노드 | 상태 | 시도 | 소요 | worker | 오류코드 | Task Run`

정렬 가능. 파이프라인 뷰가 감당 못 하는 큰 그래프의 대비책.
...
Phase 1만으로도 지금보다 압도적으로 낫다. 3·4는 표현력 강화다.
